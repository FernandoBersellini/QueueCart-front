...
describe("useShipOrder", () => {
    it("ships an order and invalidates list + item", async () => {
        server.use(
            http.patch("http://localhost:8080/order/order/:id/ship", () =>
                HttpResponse.json({
                    id: 1, userId: 7, status: "SHIPPED", items: [], totalAmount: 0,
        ...
        const { result } = renderHook(() => useShipOrder(), { wrapper: Wrapper })

        result.current.mutate(1)

        await waitFor(() => expect(result.current.isSuccess).toBe(true))

        expect(result.current.data).toMatchObject({ id: 1, status: "SHIPPED" })
        expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ["orders"] })
        expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ["order", 1] })
    })

    it("surfaces an ApiError when shipping fails", async () => {
        server.use(
            http.patch("http://localhost:8080/order/order/:id/ship", () =>
                HttpResponse.json(
