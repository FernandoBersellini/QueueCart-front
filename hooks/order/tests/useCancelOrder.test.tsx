...
describe("useCancelOrder", () => {
    it("cancels an order and invalidates list + item", async () => {
        server.use(
            http.patch("http://localhost:8080/order/order/:id/cancel", () =>
                HttpResponse.json({
                    id: 1, userId: 7, status: "CANCELLED", items: [], totalAmount: 0,
        ...
        expect(result.current.data).toMatchObject({ id: 1, status: "CANCELLED" })
        expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ["orders"] })
        expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ["order", 1] })
    })

    it("surfaces an ApiError when cancellation fails", async () => {
        server.use(
            http.patch("http://localhost:8080/order/order/:id/cancel", () =>
                HttpResponse.json(
